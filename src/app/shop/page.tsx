import Image from 'next/image'
import React from 'react'

const Shop = () => {
  return (
    <div className='h-full w-full'>
        <Image src={"/Rectangle 1.png"} height={100} width={1430} alt='img'>

        </Image>
        
        <section className="text-gray-600 body-font">
  <div className="container px-5 py-24 mx-auto">
    <div className="flex flex-wrap -m-4">
      <div className="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a className="block relative h-48 rounded overflow-hidden">
          <Image
            alt="Image" height={100} width={100}
            className="object-cover object-center w-full h-full block"
            src="/Mask group.png"
          />
        </a>
        <div className="mt-4">
          
          <p className="text-gray-900 title-font text-lg font-medium">
          Trenton modular sofa_3
          </p>
          <p className="mt-1">Rs. 25,000.00</p>
        </div>
      </div>
      <div className="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a className="block relative h-48 rounded overflow-hidden">
          <Image height={100} width={100}
            alt="img"
            className="object-cover object-center w-full h-full block"
            src="/Mask group (1).png"
          />
        </a>
        <div className="mt-4">
          
          <p className="text-gray-900 title-font text-lg font-medium">
          Granite dining table with dining chair
          </p>
          <p className="mt-1">Rs. 25,000.00</p>
        </div>
      </div>
      <div className="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a className="block relative h-48 rounded overflow-hidden">
          <Image height={100} width={100}
            alt="img"
            className="object-cover object-center w-full h-full block"
            src="/Mask group (2).png"
          />
        </a>
        <div className="mt-4">

          <p className="text-gray-900 title-font text-lg font-medium">
          Outdoor bar table and stool
          </p>
          <p className="mt-1">Rs. 25,000.00</p>
        </div>
      </div>
      <div className="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a className="block relative h-48 rounded overflow-hidden">
          <Image
            alt="img" height={100} width={100}
            className="object-cover object-center w-full h-full block"
            src="/chair.png"
          />
        </a>
        <div className="mt-4">
          
          <p className="text-gray-900 title-font text-lg font-medium">
          Plain console with teak mirror
          </p>
          <p className="mt-1">Rs. 25,000.00</p>
        </div>
      </div>
      <div className="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a className="block relative h-48 rounded overflow-hidden">
          <Image
            alt="img" height={100} width={100}
            className="object-cover object-center w-full h-full block"
            src="/Grain coffee table 1.png"
          />
        </a>                         
        <div className="mt-4">
        
          <p className="text-gray-900 title-font text-lg font-medium">
          Grain coffee table
          </p>
          <p className="mt-1">Rs. 15,000.00</p>
        </div>
      </div>
      <div className="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a className="block relative h-48 rounded overflow-hidden">
          <img
            alt="img"
            className="object-cover object-center w-full h-full block"
            src="Mask group (3).png"
          />
        </a>
        <div className="mt-4">
         
          <h2 className="text-gray-900 title-font text-lg font-medium">
          Kent coffee table
          </h2>
          <p className="mt-1">Rs. 225,000.00</p>
        </div>
      </div>
      <div className="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a className="block relative h-48 rounded overflow-hidden">
          <Image height={100} width={100}
            alt="img"
            className="object-cover object-center w-full h-full block"
            src="/Mask group (4).png"
          />
        </a>
        <div className="mt-4">
          
          <p className="text-gray-900 title-font text-lg font-medium">
          Round coffee table_color 2
          </p>
          <p className="mt-1">Rs. 251,000.00</p>
        </div>
      </div>
      <div className="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a className="block relative h-48 rounded overflow-hidden">
          <Image
            alt="img" height={100} width={100}
            className="object-cover object-center w-full h-full block"
            src="/Mask group (5).png"
          />
        </a>
        <div className="mt-4">
         
          <p className="text-gray-900 title-font text-lg font-medium">
            Reclaimed teak coffee table
          </p>
          <p className="mt-1">$18.40</p>
        </div>
      </div>
{/* add more */}



<div className="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a className="block relative h-48 rounded overflow-hidden">
          <Image height={100} width={100}
            alt="img"
            className="object-cover object-center w-full h-full block"
            src="/Mask group (6).png"
          />
        </a>
        <div className="mt-4">
          
          <p className="text-gray-900 title-font text-lg font-medium">
          Plain console_2
          </p>
          <p className="mt-1">Rs. 258,200.00</p>
        </div>
      </div>

      {/* add more */}
      <div className="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a className="block relative h-48 rounded overflow-hidden">
          <Image
            alt="img" width={100} height={100}
            className="object-cover object-center w-full h-full block"
            src="/Mask group (7).png"
          />
        </a>
        <div className="mt-4">
          
          <p className="text-gray-900 title-font text-lg font-medium">
          Trenton modular sofa_3
          </p>
          <p className="mt-1">Rs. 20,000.00</p>
        </div>
      </div>
      {/* add more */}
      <div className="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a className="block relative h-48 rounded overflow-hidden">
          <Image
            alt="img" width={100} height={100}
            className="object-cover object-center w-full h-full block"
            src="/Mask group (8).png"
          />
        </a>
        <div className="mt-4">
          
          <p className="text-gray-900 title-font text-lg font-medium">
          Trenton modular sofa_3
          </p>
          <p className="mt-1">Rs. 200,000.00</p>
        </div>
      </div>
      {/* add more */}
      <div className="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a className="block relative h-48 rounded overflow-hidden">
          <Image
            alt="img" height={100} width={100}
            className="object-cover object-center w-full h-full block"
            src="/Mask group (9).png"
          />
        </a>
        <div className="mt-4">
          
          <p className="text-gray-900 title-font text-lg font-medium">
          SJP_0825 
          </p>
          <p className="mt-1">Rs. 200,000.00</p>
        </div>
      </div>
      {/* add more */}
      <div className="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a className="block relative h-48 rounded overflow-hidden">
          <Image
            alt="img" height={100} width={100}
            className="object-cover object-center w-full h-full block"
            src="/table 1.png"
          />
        </a>
        <div className="mt-4">
          
          <p className="text-gray-900 title-font text-lg font-medium">
          Granite square side table
          </p>
          <p className="mt-1">Rs. 258,800.00</p>
        </div>
      </div>
      {/* add more */}
      <div className="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a className="block relative h-48 rounded overflow-hidden">
          <Image
            alt="img" height={100} width={100}
            className="object-cover object-center w-full h-full block"
            src="/Mask group (11).png"
          />
        </a>
        <div className="mt-4">
          
          <p className="text-gray-900 title-font text-lg font-medium">
          Asgaard sofa
          </p>
          <p className="mt-1">Rs. 250,000.00</p>
        </div>
      </div>
      {/* add more */}
      <div className="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a className="block relative h-48 rounded overflow-hidden">
          <Image
            alt="img" height={100} width={100}
            className="object-cover object-center w-full h-full block"
            src="/Mask group (12).png"
          />
        </a>
        <div className="mt-4">
          
          <p className="text-gray-900 title-font text-lg font-medium">
          Maya sofa three seater
          </p>
          <p className="mt-1">Rs. 115,000.00</p>
        </div>
      </div>
      {/* add more */}
      <div className="lg:w-1/4 md:w-1/2 p-4 w-full">
        <a className="block relative h-48 rounded overflow-hidden">
          <Image
            alt="img" height={100} width={100}
            className="object-cover object-center w-full h-full block"
            src="/Mask group (13).png"
          />
        </a>
        <div className="mt-4">
          
          <p className="text-gray-900 title-font text-lg font-medium">
          Outdoor sofa set
          </p>
          <p className="mt-1">Rs. 244,000.00</p>
        </div>
      </div>
    
    

      





    </div>
{/* // add other saction// */}
<section className="text-gray-600 body-font bg-[#FAF4F4] mt-10">
  <div className="container px-5 py-24 mx-auto">
    <div className="flex flex-wrap -m-4 text-center">
      <div className="p-4 sm:w-1/3 w-1/2">
        <h2 className="title-font font-medium sm:text-4xl text-3xl text-gray-900">
        Free Delivery
        </h2>
        <p className="leading-relaxed text-sm text-[#9F9F9F] pt-3">For all oders over $50, consectetur <br /> adipim scing elit.</p>
      </div>
      <div className="p-4 sm:w-1/4 w-1/2">
        <h2 className="title-font font-medium sm:text-4xl text-3xl text-gray-900">
        90 Days Return
        </h2>
        <p className="leading-relaxed text-sm text-[#9F9F9F] pt-3">If goods have problems, consectetur adipim scing elit.</p>
      </div>
      <div className="p-4 sm:w-1/4 w-1/2">
        <h2 className="title-font font-medium sm:text-4xl text-3xl text-gray-900">
        Secure Payment
        </h2>
        <p className="leading-relaxed text-sm text-[#9F9F9F] pt-3">100% secure payment, consectetur adipim scing elit.</p>
      </div>
    
    </div>
  </div>
</section>






  </div>
</section>

    </div>
    






  )
}

export default Shop

